...
def main():
    ...
    parser.add_argument("--ec_label", required=True, help="EC number")
    parser.add_argument("--output_dir", required=True, help="Output directory")
    parser.add_argument(
        "--validation_split", type=int, default=10, help="Validation split percentage"
    )
    ...
    for i in sequences_list:
        ...
        available_space = (
            1021 - control_code_length
        )  # It is not 1024 because '<|endoftext|>', and start and end
    ...
    processed_file = f"{args.output_dir}/{ec_label}_processed.txt"
    fn = open(processed_file, "w")
    ...
    raw_datasets = load_dataset(
        extension, data_files=data_files, cache_dir=args.output_dir, **dataset_args
    )
